...
| <a name="input_scan_cron_expression"></a> [scan\_cron\_expression](#input\_scan\_cron\_expression) | When the scan runs, in UTC. Five fields, no seconds field, so the wall-clock time shifts with British Summer Time. Sunday evening by default, so the digest is waiting on Monday morning but is measured after a full week of Renovate merges rather than an hour into Monday's burst. | `string` | `"0 18 * * 0"` | no |
